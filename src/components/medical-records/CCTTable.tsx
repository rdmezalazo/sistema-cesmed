...
export function CCTTable({ formData, onFieldChange, section }: CCTTableProps) {
  ...
  const findFieldId = (eye: 'od' | 'oi') => {
    ...
    // Prefer fields that mention the eye plus context keywords (cct/um/grosor/corneal/central)
    const contextTokens = ['cct', 'um', 'micra', 'micras', 'grosor', 'corneal', 'central', 'espesor'];
    ...
    // Fallback: any field that at least includes the eye token
    if (!match) {
  ...
  const renderCell = (eye: 'od' | 'oi') => {
    ...
    return (
      <Input
        ...
        onBlur={handleBlur}
        inputMode="decimal"
        className="h-8 text-center bg-accent/5"
        placeholder="um"
      />
    );
  };
